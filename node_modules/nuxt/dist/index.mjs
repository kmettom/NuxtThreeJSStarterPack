    ...
    transform(code, id) {
      ...
      walk(this.parse(code, { sourceType: "module", ecmaVersion: "latest" }), {
        enter: (_node) => {
          ...
          } else if (node.type === "CallExpression" && node.callee.type === "Identifier" && SSR_RENDER_RE.test(node.callee.name)) {
              ...
              if (isClientComponent && children?.type === "ObjectExpression") {
                ...
                for (const slot of slotsToRemove) {
                  ...
                  walk(this.parse(removedCode, { sourceType: "module", ecmaVersion: "latest" }), {
                    enter: (_node2) => {
                      const node2 = _node2;
                      if (node2.type === "CallExpression" && node2.callee.type === "Identifier" && SSR_RENDER_RE.test(node2.callee.name)) {
                        const componentNode = node2.arguments[0];
                        if (componentNode.type === "CallExpression") {
                          const identifier = componentNode.arguments[0];
                          if (!isRenderedInCode(currentCode, removedCode.slice(componentNode.start, componentNode.end))) {
                          ...
                          const { start, end } = componentNode;
                          if (!isRenderedInCode(currentCode, removedCode.slice(start, end))) {
                            componentsSet.add(componentNode.property.value);
                            walk(this.parse(code, { sourceType: "module", ecmaVersion: "latest" }), {
                              enter: (node3) => {
                                removeFromSetupReturnStatement(s, node3, componentNode.property.value);
                              }
                            });
                          }
                        }
                  ...
                  for (const componentName2 of componentsToRemove) {
                    let removed = false;
                    const VAR_RE = new RegExp(`(?:const|let|var) ${componentName2} = ([^;\\n]*);?`);
                    s.replace(VAR_RE, () => {
                      removed = true;
                      return "";
                    });
                    if (!removed) {
                      const declaration = findImportDeclaration(importDeclarations, componentName2);
                      if (declaration) {
                        if (declaration.specifiers.length > 1) {
                          const componentSpecifier = declaration.specifiers.find((s2) => s2.local.name === componentName2);
                          if (componentSpecifier) {
                            s.remove(componentSpecifier.start, componentSpecifier.end + 1);
                          }
                        } else {
...
function findImportDeclaration(declarations, importName) {
...
function isRenderedInCode(code, name) {
  return new RegExp(`ssrRenderComponent\\(${escapeRE(name)}`).test(code);
}
...
function removeFromSetupReturnStatement(s, node, name) {
  if (node.type === "Property" && node.key.type === "Identifier" && node.key.name === "setup" && node.value.type === "FunctionExpression") {
    const returnStatement = node.value.body.body.find((n) => n.type === "ReturnStatement");
    if (returnStatement?.argument?.type === "Identifier") {
      const returnIdentifier = returnStatement.argument.name;
      const returnedDeclaration = node.value.body.body.find((n) => n.type === "VariableDeclaration" && n.declarations[0].id.name === returnIdentifier);
      const componentProperty = returnedDeclaration?.declarations[0].init?.properties.find((p) => p.key.name === name);
      if (componentProperty) {
        s.remove(componentProperty.start, componentProperty.end + 1);
      }
    } else if (returnStatement?.argument?.type === "ObjectExpression") {
      const componentProperty = returnStatement.argument?.properties.find((p) => p.key.name === name);
      if (componentProperty) {
        s.remove(componentProperty.start, componentProperty.end + 1);
      }
    }
